...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function(exports){
    ...
    var $ = layui.$
        ...
        ,getThisTable = function(id){
            var table = thisTable.that;
            if(!table) hint.error('The ID option was not found in the table instance');
            return table || null;
        }
        ...
        ,TPL_HEADER = function(options){
            ...
            return ['<table cellspacing="0" cellpadding="0" border="0" class="layui-table" '
                ...
                ,'<th data-field="{{ item2.field||i2 }}" data-key="{{d.index}}-{{i1}}-{{i2}}" {{# if( item2.parentKey){ }}data-parentkey="{{ item2.parentKey }}"{{# } }} {{# if(item2.fixedWidth){ }}style="width:{{item2.fixedWidth}}px"{{# } }} {{# if(item2.minWidth){ }}data-minwidth="{{item2.minWidth}}"{{# } }} '+ rowCols +' {{# if(item2.unresize || item2.colGroup){ }}data-unresize="true"{{# } }} class="{{# if(item2.hide){ }}layui-hide{{# } }}{{# if(isSort){ }} layui-unselect{{# } }}{{# if(!item2.field){ }} layui-table-col-special{{# } }}">'
        ...
        if(options.url){ //Ajax请求
            ...
            $.ajax({
                ...
                ,success: function(res){
                    ...
                    //如果不允许1970的时间出现则设置一个默认值
                    if(!options.allowNullTime){
                        var list = (res&&res.rows) || (res&&res.data)
                        if(list&&list.length){
                            $.each(list,function () {
                                for (var key in this) {
                                    var el = this[key]
                                    if(typeof el == 'string' && el.indexOf('1970-01-01') > -1){
                                        this[key] = '---'
                                    }
                                }
                            })
                    ...
                    if(typeof options.parseData === 'function'){
                        res = options.parseData(res,data) || res;
                    }
                    //检查数据格式是否符合规范
            ...
            ,render = function(){ //后续性能提升的重点
                ...
                layui.each(data, function(i1, item1){
                    ...
                    that.eachCols(function(i3, item3){
                        ...
                        //td内容
                        //新增fixedWidth sola
                        var fixedWidth = item3.fixedWidth ? 'style="width: '+item3.fixedWidth+'px"' : ""

                        var td = ['<td '+fixedWidth+' data-field="'+ field +'" data-key="'+ key +'" '+ function(){   //追加各种属性
                            var attr = [];
    ...
    Class.prototype.renderTotal = function(data,res){
        ...
        that.eachCols(function(i3, item3){
            ...
            var td = ['<td data-field="'+ field +'" data-key="'+ options.index + '-'+ item3.key +'" '+ function(){
                ...
                ,'<div class="layui-table-cell text-primary laytable-cell-'+ function(){ //返回对应的CSS类标识
                    //加入合计懒加载行  - sola
                    var str = (options.index + '-' + item3.key + ' table-cell-lazy-' + field);
                    return item3.type === 'normal' ? str
                        : (str + ' laytable-cell-' + item3.type);
                }() +'">' + function(){
                    var text = item3.totalRowText || '';
                    //加入合计懒加载行 - sola
                    if(item3.totalRowLazy){
                        return '<span class="mdi mdi-spinner" style="animation: rotation-360 1.4s linear infinite;font-size:18px;position:relative;top:2px"></span> <span>加载中...</span>'
                    }
                    return item3.totalRow ? (formatMoney(totalNums[field]) || text || 0) : text;
                }()
                ,'</div></td>'].join('');
    ...
    Class.prototype.setThisRowChecked = function(index){
        ...
        if(!that.config.clickToSelect){
            tr.addClass(ELEM_CLICK).siblings('tr').removeClass(ELEM_CLICK);
        }else{
            if(that.layFixLeft.length){
                var $current = that.layFixLeft.find('tr[data-index="'+ index +'"] input[name="layTableCheckbox"]+')
                $current.trigger("click")
                var $siblings = that.layFixLeft.find('tr[data-index="'+ index +'"]').siblings()
                $siblings.find('input[name="layTableCheckbox"]:checked+').trigger("click")
            }else{
                tr.find('input[name="layTableCheckbox"]+').trigger("click")
            }
        }
        //sola 选中状态
        // var checkAllElem = tr.find('input[name="layTableCheckbox"]+')
        // if(tr.hasClass(ELEM_CLICK)){
        //     tr.removeClass(ELEM_CLICK)
        // }else{
        //     tr.addClass(ELEM_CLICK)
        // }
        // checkAllElem.length&&checkAllElem.click()
    };
    ...
    Class.prototype.loading = function(hide){
        ...
        if(options.loading){
            if(hide){
                ...
                that.layInit = $(['<div class="layui-table-init"><div class="be-loading be-loading-active" style="height:100%">'
                    ,'<div class="be-spinner"><svg width="40px" height="40px" viewBox="0 0 66 66" xmlns="http://www.w3.org/2000/svg">\n' +
    ...
    Class.prototype.setCheckData = function(index, checked){
        var that = this
            ...
            ,ELEM_CLICK = 'layui-table-click'
            ,tr = that.layBody.find('tr[data-index="'+ index +'"]')
            ,fixedTr = that.layFixed.find('tr[data-index="'+ index +'"]')

        if(!thisData[index]) return;
        if(thisData[index].constructor === Array) return;
        thisData[index][options.checkName] = checked;
        //sola
        if(checked&&that.config.clickToSelect){
            tr.addClass(ELEM_CLICK)
        }else if(!checked&&that.config.clickToSelect){
            // console.log(fixedTr.find(".layui-form-checkbox"))
            // console.log( tr.find(".layui-form-checkbox"))
            // setTimeout(function () {
            // fixedTr.find(".layui-form-checkbox").removeClass("layui-form-checked")
            // },100)
            tr.removeClass(ELEM_CLICK)
        }
    ...
    Class.prototype.fullSize = function(){
        ...
        //如果table有赋值则使用table的高度赋值 - -sola
        options.height = table.resizeHeight ? table.resizeHeight : options.height
        if(options.height && /^full-\d+$/.test(options.height)){
            that.fullHeightGap = options.height.split('-')[1];
            options.height = _WIN.height() - that.fullHeightGap;
        }
    ...
    Class.prototype.scrollPatch = function(){
        ...
        // //检测表格内容滚动条是否出现 sola
        // var tableHeader = $(that.layHeader[0])
        // //如果有纵向滚动条加入填充宽度 sola
        // var $tr = tableHeader.find("thead tr:first")



        // var paddingTh = $tr.find(".zl-padding-th")
        //
        // console.log(paddingTh)
        // if(bodyHeight<bodyScrollHeight){
        //     console.log(that.layTotal)
        //     paddingTh.length ? null : $tr.append("<th class='zl-padding-th' width='"+(scollWidth-1) +"'></th>")
        //     that.layTotal.append("<td class='zl-padding-th' width='"+(scollWidth-1) +"'></td>")
        // }else{
        //     paddingTh.length?  paddingTh.remove() : null
        // }
    };
        ...
        that.layTool.on('click', '*[lay-event]', function(e){
            ...
            switch(events){
                ...
                case 'LAYTABLE_PRINT': //打印
                    ...
                    printWin.close();
                    break;
                case 'LAYTABLE_PRINT': //sola 导入
                    //sola
                    if(that.config.input){
                        that.config.input()
                        return
                    }
                    //
